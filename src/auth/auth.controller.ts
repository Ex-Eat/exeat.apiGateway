...
export class AuthController {
    ...
    async user(@Req() req): Promise<IUserDto> {
        let user;
        try {
            user = await lastValueFrom(this._service.getLoggedUser(req.cookies.access_token));
        } catch (e: any) {
            if (e.message === HttpErrorsEnum.TOKEN_EXPIRED_REFRESH_NEEDED) {
                const tokens = await lastValueFrom(this._service.refreshToken(req.cookies.access_token, req.cookies.refresh_token));
                user = tokens.user;
            } else {
    ...
    @Post('signup')
    async signup(@Body() user: ICreateUserDto, @Res({ passthrough: true }) res) {
